...
class ClipExtractor(torch.nn.Module):
    def __init__(self, device, cfg=None):
        super().__init__()
        self.cfg = cfg
        clip_model_name = 'ViT-B/32'  # Keep the original model architecture
        clip_affine_transform_fill = 1
        n_aug = 16
        
        # Load the original model first (this preserves the expected interface)
        original_model = clip.load(clip_model_name, device=device)[0]
        
        # Now load your fine-tuned model's weights
        model_path = "./best_model_accuracy.pt"
        checkpoint = torch.load(model_path, torch.device(device), weights_only=True)
        ...
        for key, value in model_state_dict.items():
            ...
            elif not key.startswith('matching_head'):
                fixed_state_dict[key] = value
        
        # Load the state dict with strict=False to preserve the interface
        original_model.load_state_dict(fixed_state_dict, strict=False)
        
        # Assign the model exactly as in the original code
        self.model = original_model.eval().requires_grad_(False)

        # Keep all the rest of the initialization exactly the same
